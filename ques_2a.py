...
class Alice:
    ...
    def play_move(self):
        if self.results[-1] == 0:
            return 1  # Bob won the previous round, play balanced as the expectation of points come maximum that ways
        if self.results[-1] == 0.5:
            return 0  # Previous round was a draw, play aggressive as the expectation of points come maximum that ways
        if self.results[-1] == 1:
            if ((len(self.results) - self.points) * 11) > (6 * (len(self.results))):
                return 0  # Play aggressive when behind as the expectation of points come maximum that ways
            else:
                return 2  # Play defensive when ahead as the expectation of points come maximum that ways
...
def simulate_round(alice, bob, payoff_matrix):
    ...
    result = np.random.choice([1, 0.5, -1], p=[p_win, p_draw, p_lose]) # function to randomly generate the outcome of the event based on probabilities 
    if result == 1:
        alice_result = 1
        bob_result = 0
...
if __name__ == "__main__":
    ...
    # look at the defence defence cell of the matrix defence mein defence krdo
